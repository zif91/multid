...
public function makeSQLForSelectWords ()
{
    ...
    $where .= ($this->params['includeTmpls'] != '' ? ' AND template IN(' . $this->params['includeTmpls'] . ') ' : '');
...
public function emptyExcluded()
{
    //if (isset($this->params['reindex']) && (int)$this->params['reindex'] != 0) {
        $where = '';
        $ids = array();
        if($this->params['includeTmpls'] != '') {
            $where = ' template NOT IN(' . $this->cleanIn($this->params['includeTmpls']) . ') ';
        } else {
...
public function injectTVs($id, $content)
{
    ...
    if ($this->params['TvNames'] != '') {
        ...
        $TvNames = array_map('trim', $TvNames);
