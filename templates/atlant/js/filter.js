...
var Filter = {
    ...
    selectedCars: [],
		init: function() {
			...
			$(document).on('change', '[name="f[42]"]', function(e) {
				e.preventDefault();
				if (Filter.selectedCars.includes('43')) {
					$('[name="f[43]"]').prop('selectedIndex', 0);
					Filter.selectedCars.splice(Filter.selectedCars.indexOf('43'), 1);
				}
				if ($(this).val()) {
					Filter.selectedCars.push('42');
				} else {
					Filter.selectedCars.splice(Filter.selectedCars.indexOf('42'), 1);
				}
				Filter.build();
			});

			$(document).on('change', '[name="f[43]"]', function(e) {
				e.preventDefault();
				if (Filter.selectedCars.includes('42')) {
					$('[name="f[42]"]').prop('selectedIndex', 0);
					Filter.selectedCars.splice(Filter.selectedCars.indexOf('42'), 1);
				}
				if ($(this).val()) {
					Filter.selectedCars.push('43');
				} else {
					Filter.selectedCars.splice(Filter.selectedCars.indexOf('43'), 1);
				}
				Filter.build();
			...
			$(document).on('click', '.Button--reset', function(e) {
				...
				$(document).on("change", ".range-min", function() {
					...
					$(this).closest('.price-range').find('.filter-range-slider').data('ionRangeSlider').update({
						from: val
					});
					Filter.build();
				});
